...
serve(async (req) => {
  ...
  try {
    ...
    if (projErr || !project)
      throw new Error("Project not found: " + (projErr?.message ?? "unknown"));
    ...
    if (
      ...
      try {
        ...
        if (result.transcript) {
          ...
          console.error(
            "fetch-transcript returned no transcript:",
            result.error,
          );
        }
      } catch (fetchErr) {
    ...
    if (!transcript || transcript.trim().length < 10) {
      ...
      return new Response(
        JSON.stringify({
          error:
            "Could not extract transcript. Please paste it manually on the project page.",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    for (const formatType of selectedOutputs) {
      try {
        ...
        if (result.content) {
          ...
          console.error(
            `generate-content failed for ${formatType}:`,
            result.error,
          );
        }
      } catch (genErr) {
    ...
    if (successCount === 0) {
      ...
      return new Response(
        JSON.stringify({
          error: "Content generation failed for all formats. Check AI_API_KEY.",
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
