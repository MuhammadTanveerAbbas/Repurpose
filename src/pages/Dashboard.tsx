          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
            <div>
              ...
              <p className="font-sans text-sm text-stone-400 mt-0.5">
                {totalOutputs} pieces created · {thisMonth} project
                {thisMonth !== 1 ? "s" : ""} this month
              </p>
            </div>
            <Button
              size="sm"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            {[
              ...
              },
              {
                ...
                icon: <Youtube className="h-5 w-5 text-amber-400" />,
              },
              {
                label: "Plan",
                value: profile?.plan ?? "free",
                icon: <Upload className="h-5 w-5 text-amber-400" />,
              },
            ].map((stat) => (
              <div
                key={stat.label}
                className="bg-white border border-stone-100 rounded-2xl p-5 shadow-sm flex flex-col gap-1"
              >
                ...
                <p className="font-display text-3xl text-stone-900 capitalize">
                  {stat.value}
                </p>
              </div>
            ))}
            ...
            <div className="rounded-2xl border border-stone-100 bg-white p-5 mb-8 shadow-sm">
              <div className="flex items-center justify-between mb-2">
                <span className="font-sans text-sm font-medium text-stone-700">
                  Free plan · {used}/{limit} projects used
                </span>
                <Link
                  to="/pricing"
                  className="font-sans text-xs font-medium text-amber-600 hover:underline"
                >
                  Upgrade
                </Link>
              </div>
        ...
        <FadeUp delay={150}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-sans font-semibold text-stone-900 text-lg">
              Your projects
              {projects.length > 0 && (
                <span className="ml-2 font-sans text-xs font-medium bg-stone-100 text-stone-500 rounded-full px-2 py-0.5">
                  {projects.length}
                </span>
              )}
            </h2>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <p className="font-sans text-sm text-stone-400 mb-6 max-w-xs leading-relaxed">
                Paste a YouTube URL or upload audio to create your first content
                pieces.
              </p>
              <Button
                size="sm"
              ...
              {projects.map((project, i) => (
                <FadeUp key={project.id} delay={i * 50}>
                  ...
                  >
                    ...
                    <p className="font-sans font-semibold text-stone-900 text-sm truncate mb-1">
                      {project.title}
                    </p>
                    <p className="font-sans text-xs text-stone-400">
                      ...
                      · {project.output_count} output
                      {project.output_count !== 1 ? "s" : ""}
                    </p>
                    <div className="flex gap-1.5 mt-3 flex-wrap">
                      <span className="font-sans text-[10px] px-2 py-0.5 rounded-full bg-stone-100 text-stone-500 capitalize">
                        {project.source_type}
                      </span>
                    </div>
                  </Link>
