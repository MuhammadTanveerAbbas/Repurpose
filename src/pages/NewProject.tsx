...
const outputFormats = [
  {
    id: "linkedin_long",
    label: "LinkedIn Post (long-form)",
    desc: "1200-1500 word thought leadership",
  },
  {
    id: "linkedin_hook",
    label: "LinkedIn Post (short hook)",
    desc: "Punchy scroll-stopping opener",
  },
  {
    id: "email_newsletter",
    label: "Email Newsletter Section",
    desc: "Ready-to-send newsletter block",
  },
  {
    id: "twitter_thread",
    label: "Twitter/X Thread (7 tweets)",
    desc: "Under 280 chars each",
  },
  {
  ...
  {
    id: "short_form_scripts",
    label: "Short-form Video Scripts",
    desc: "3 × 60-second hooks",
  },
];

const NewProject = () => {
  const [step, setStep] = useState(1);
  const [sourceType, setSourceType] = useState<"youtube" | "upload" | "paste">(
    "youtube",
  );
  ...
  const handleCreate = async () => {
    ...
    // Fire-and-forget processing runs in the background edge function
    supabase.functions
      .invoke("process-project", {
        body: { projectId },
      })
  ...
  const inputClass =
    "h-11 rounded-xl border-stone-200 bg-white text-stone-900 placeholder:text-stone-400 focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400 font-sans";
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {["Input", "Details", "Outputs"].map((label, i) => (
            <div key={label} className="flex items-center gap-2">
              ...
              <span
                className={cn(
                  "text-sm font-medium hidden sm:inline font-sans",
                  i + 1 <= step ? "text-stone-900" : "text-stone-400",
                )}
              >
                {label}
              </span>
              {i < 2 && (
                <div
                  className={cn(
                    ...
                    i + 1 < step ? "bg-[#E8743A]" : "bg-stone-200",
                  )}
                />
              )}
            </div>
        ...
        {step === 1 && (
          <div className="bg-white border border-stone-100 rounded-2xl shadow-sm p-6">
            ...
            <p className="font-sans text-sm text-stone-400 mb-6">
              Paste a YouTube URL, upload a file, or paste a transcript directly
            </p>
            ...
            {sourceType === "youtube" && (
              <div className="space-y-2 mb-6">
                ...
                <p className="font-sans text-xs text-stone-400">
                  We'll try to auto-extract the transcript. If it fails, you can
                  paste it manually.
                </p>
              </div>
            )}
            ...
            {sourceType === "paste" && (
              <div className="space-y-2 mb-6">
                ...
                <div className="flex items-center justify-between">
                  <p className="font-sans text-xs text-stone-400">
                    Tip: Copy the transcript from YouTube's "Show transcript"
                    feature.
                  </p>
                  <span
                    className={cn(
        ...
        {step === 3 && (
          <div className="bg-white border border-stone-100 rounded-2xl shadow-sm p-6">
            ...
            <p className="font-sans text-sm text-stone-400 mb-6">
              ...
              {profile?.plan === "free" && (
                <span className="text-amber-600">Upgrade for all 6.</span>
              )}
            </p>
            ...
            <div className="grid gap-3 mb-6">
              {outputFormats.map((format) => {
                ...
                const disabled =
                  !selected && selectedOutputs.length >= maxOutputs;
                return (
                  <div
                    key={format.id}
                    ...
                    onKeyDown={(e) =>
                      e.key === " " && !disabled && toggleOutput(format.id)
                    }
                    className={cn(
                  ...
                  >
                    ...
                    <div>
                      <p className="font-sans font-semibold text-sm text-stone-800">
                        {format.label}
                      </p>
                      <p className="font-sans text-xs text-stone-400">
                        {format.desc}
